...
func Presale(c *gin.Context, bolt *repository.Bolt) {
	...
	req, _ := http.NewRequest("POST", baseURL+"/api/Payment/PreSale", bytes.NewBuffer(saleReqJson))
	...
	err = bolt.TransactionRepo.LogRequest("presale", "request", saleReq.OrderID, saleReqJson, req.Header)
	if err != nil {
		log.Panic(err)
	}
	...
	err = bolt.TransactionRepo.LogRequest("presale", "response", saleReq.OrderID, response.Result, resp.Header)
