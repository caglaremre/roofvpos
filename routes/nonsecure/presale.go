...
func Presale(c *gin.Context, bolt *repository.Bolt) {
	...
	err = bolt.TransactionRepo.Log("presale", "request", saleReq.OrderID, saleReqJson, req.Header)
	if err != nil {
		log.Panic(err)
	}
	...
	err = bolt.TransactionRepo.Log("presale", "response", saleReq.OrderID, response.Result, resp.Header)
